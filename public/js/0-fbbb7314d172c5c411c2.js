...
function loopCallback(i, files, self) {
	function loop(i, files) {
		if (i < files.length) {
			$.getScript(files[i]).done(function (script, textStatus) {
				//loopCallback(i+1);
			}).fail(function (jqxhr, settings, exception) {
				self.setState({ isLoaded: 'chet tiet' });
				return loop(i + 1, files);
			});
			return false;
		} else {
			return true;
		...
		var Script = function (_Component) {
			...
			_createClass(Script, [{
				...
				value: function componentDidMount() {
					loopCallback(0, params, this).then(function (file) {
						console.log(file);
					});
				}
			}, {
