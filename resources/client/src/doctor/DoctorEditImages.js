...
import * as DoctorFormImagesActions from './actions/formImages';
...
class DoctorEdit extends Component{
	...
	componentDidMount(){
		...
		.then(data => {
			_this.props.doctorFormImagesFill(data.doctors_images);
	...
	componentWillUnmount(){
		this.props.doctorFormImagesClear();
		this.state = {
			confirm: false
		}
	}
	_loopCallbackImages(i, files){
		...
		if(i < files.length){
			...
			this.props.doctorFormImagesUpload(data)
			.then(data => {
	...
	_onRemoveServerImage(image){
		this.props.doctorFormImagesChoose(image);
		this.setState({confirm: true});
	}
	_onUploadAll(event){
		event.preventDefault();
		let _this = this;
		const images = GetFilesUpload(this.props.doctorFormImages.values.images);
		if(images.length === 0){
	...
	_onUploadRemove(){
		...
		const image = this.props.doctorFormImages.image;
		const imageWrapper = document.getElementById(image.id);
		this.props.themeShowLoadingEl(imageWrapper);
		this.props.doctorFormImagesRemove(image.id)
		.then(data => {
			_this.props.themeHideLoadingEl(imageWrapper);
			_this._refreshPage();
		})
										...
										<div className="tab-pane active">
											<form method="POST" noValidation encType="multipart/form-data" ref="form">
												<div className="form-body">
													<MultipleImageUpload
														onChange={images => this.props.doctorFormImagesChange('images', images)}
														onRemove={this._onRemoveServerImage.bind(this)}
														images={this.props.doctorFormImages.values.images}/>
												</div>
...
const mapStateToProps = ({doctor, doctorFormImages}) => {
	return {doctor, doctorFormImages};
};
