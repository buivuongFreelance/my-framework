...
class BlogNew extends Component{
	...
	_onSubmit(event){
		...
		setTimeout(() => {
			_this._onFocusField();
			let values = $.extend({}, _this.props.blogFormNew.values);
			...
			if(valid){
				...
				_this.props.blogFormNewSubmit(values)
				.then(data => {
					_this.props.themeHideLoadingEl(_this.refs.form);
					_this.props.themeShowSuccess('Create New Blog Successfully');
					_this.props.push(Routes.backend.blogList);
				})
				.catch(message => {
					_this.props.themeHideLoadingEl(_this.refs.form);
...
const mapStateToProps = ({blogFormNew, userAuth}) => {
	return {blogFormNew, userAuth};
};
